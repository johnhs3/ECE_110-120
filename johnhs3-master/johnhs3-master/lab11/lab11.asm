...
; R6 and 7 also contain 0
; Edited by John Smith. 4-22-2020
 
.ORIG      x3000
 
            LD    R1,INPUT            ; R1 contains input number
            LD    R2,INITF            ; R2 contains -1
            ADD   R3,R1,R2            ; R3 contains input number -1
            ADD   R3,R3,R2           ; R3 contains input number -2
                                      ;   (initializes outer count)
		AND R4,R4,#0
		AND R5,R5,#0		;Initialize all registers to zero, just to be sure.
		AND R0,R0,#0
		AND R6,R6,#0
		AND R7,R7,#0

OUTERLOOP   ADD   R4,R0,R3            ; Copy outer count into inter count
...
INNERLOOP   ADD   R5,R5,R1            ; Increment sum
            ...
            LD    R5,INIT0            ; Clear R5 (previous sum) to 0
            ...
            TRAP x25		; IS HALT SOMEHOW DIFFERENT FROM TRAP x25???
 
INPUT      .FILL  x0005               ; Input for X!, in this case X = 5
INIT0      .FILL  x0000               ; Can be used to initialize registers
INITF      .FILL  xFFFF               ; 2's complement of 1 (i.e. -1)
RESULT     .FILL  x30FF               ; At program completion, the result is stored here (x30FF)
.END
